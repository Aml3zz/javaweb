...
arrayList = queryData.statisticsByMeassage("",currentPage);
%>
...
<table width="768" border="1" align="center" cellpadding="1" cellspacing="0" bordercolordark="#ffffff" bordercolor="#ffffff" bordercolorlight="#0082BF">
  ...
  <tr>
	...
	<td align="center"><a href="checkDeleteMessage.jsp?id=<%=hashMap.get("id")%>">删除</a></td>
  </tr>
  <%}} else {%>
  <tr>
  ...
  <tr>
    <td colspan = "7">总记录数：<%=count %>&nbsp;总页数：<%=pageCount %>&nbsp;
  	当前页：<%=currentPage%>&nbsp;&nbsp;<%if (pages == 1) {%>首页<%}else {%><a href="deleteMessage.jsp?pages=1">首页</a><%}%>
  	&nbsp;&nbsp;<%if (currentPage > 1) {%><a href="deleteMessage.jsp?pages=<%=currentPage-1%>">上一页</a><%}
  	else {%>上一页<%}%>&nbsp;&nbsp;<%if (currentPage < pageCount) { %><a href="deleteMessage.jsp?pages=<%=currentPage+1%>">下一页</a><%} else{%>
  	下一页<%}%>&nbsp;&nbsp;<%if (currentPage == pageCount || currentPage > pageCount) {%>末页<%} else {%><a href="deleteMessage.jsp?pages=<%=pageCount%>">末页</a><%}%>
  	</td>
  </tr>
</table>
</body>
