...
<table width = "100%" border = "0" cellpadding = "0" cellspacing = "0">
	<tr>
		<td width = "100" height = "25" align = "center" valign="middle"><a href="deleteBusInfo.jsp"><strong>删除车次</strong></a></td>
    	<td width="4" bgcolor="#F0F7FD"></td>
		<td width = "100" height = "25" align = "center" valign="middle" bgcolor="#0082BF"><strong>删除站点</strong></td>
		<td width="4" bgcolor="#F0F7FD"></td>
...
<table width="768" border="0" cellpadding="0" cellspacing="0" align="center">
	<tr>
		<td>
		  <form name="form1" method="post" action="deleteStInfo.jsp">选择车号：
...
<table width="768" border="1" align="center" cellpadding="1" cellspacing="0" bordercolordark="#ffffff" bordercolor="#ffffff" bordercolorlight="#0082BF">
  ...
  <tr>
	...
	<td align="center"><a href="checkDeleteStInfo.jsp?busNum=<%=hashMap.get("BusNum")%>&storder=<%=hashMap.get("StOrder") %>">删除</a></td>
  </tr>
  <%}%>
  <tr>
  	<td colspan = "7">总记录数：<%=count %>&nbsp;&nbsp;总页数：<%=pageCount %>&nbsp;&nbsp;
  	当前页：<%=currentPage%>&nbsp;&nbsp;<%if (pages == 1) {%>首页<%}else {%><a href="deleteStInfo.jsp?pages=1&selectBusNum=<%=busNum%>">首页</a><%}%>
  	&nbsp;&nbsp;<%if (currentPage > 1) {%><a href="deleteStInfo.jsp?pages=<%=currentPage-1%>&selectBusNum=<%=busNum%>">上一页</a><%}
  	else {%>上一页<%}%>&nbsp;&nbsp;<%if (currentPage < pageCount) { %><a href="deleteStInfo.jsp?pages=<%=currentPage+1%>&selectBusNum=<%=busNum%>">下一页</a><%} else{%>
  	下一页<%}%>&nbsp;&nbsp;<%if (currentPage == pageCount || currentPage > pageCount) {%>末页<%} else {%><a href="deleteStInfo.jsp?pages=<%=pageCount%>&selectBusNum=<%=busNum%>">末页</a><%}%>
  	</td>
  </tr>
  <%} else {%>
  <tr>
